...
class ShopProvider extends PureComponent<MainProvidersPropTypes, any> {
  private readonly bloc: ShopBloc;

  constructor(props: MainProvidersPropTypes) {
    super(props);
    this.bloc = new ShopBloc(this.props.outletContext as OutletContextType);
  }
  ...
  handleSnapshotChange = (snapshot: any): JSX.Element => {
    if (snapshot.data && snapshot.connectionState !== -1) {
      return (
        <>
          <ShopView data={snapshot.data} bloc={this.bloc} />
