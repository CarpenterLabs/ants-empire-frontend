...
class FaucetProvider extends PureComponent<MainProvidersPropTypes, any> {
  private readonly bloc: FaucetBloc;

  constructor(props: MainProvidersPropTypes) {
    super(props);
    this.bloc = new FaucetBloc(this.props.outletContext as OutletContextType);
  }
  ...
  handleSnapshotChange = (snapshot: any): JSX.Element => {
    if (snapshot.data && snapshot.connectionState !== -1) {
      return (
        <>
          <FaucetView data={snapshot.data} bloc={this.bloc} />
