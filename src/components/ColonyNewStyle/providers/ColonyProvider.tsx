...
class ColonyProvider extends PureComponent<MainProvidersPropTypes, any> {
  private readonly bloc: ColonyBloc;

  constructor(props: MainProvidersPropTypes) {
    super(props);
    this.bloc = new ColonyBloc(this.props.outletContext as OutletContextType);
  }
  ...
  handleSnapshotChange = (snapshot: any): JSX.Element => {
    if (snapshot.data && snapshot.connectionState !== -1) {
      return (
        <>
          {/* <ColonyView data={snapshot.data} bloc={this.bloc} /> */}
          <ColonyView data={snapshot.data} bloc={this.bloc} />
