...
import ColoniesView from '../views/ColoniesView';
...
class ColoniesProvider extends PureComponent<MainProvidersPropTypes, any> {
  private readonly bloc: ColoniesBloc;

  constructor(props: MainProvidersPropTypes) {
    super(props);
    this.bloc = new ColoniesBloc(this.props.outletContext as OutletContextType);
  }
  ...
  handleSnapshotChange = (snapshot: any): JSX.Element => {
    if (snapshot.data && snapshot.connectionState !== -1) {
      return (
        <>
          <ColoniesView data={snapshot.data} bloc={this.bloc} />
...
export default ColoniesProvider;
